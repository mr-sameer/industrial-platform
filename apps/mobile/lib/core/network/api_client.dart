...
class ApiClient {
  ...
  /// For multipart/form-data uploads (Module 3B: logo, cover image,
  /// verification documents). `fields` are plain form fields (e.g.
  /// document_type); `fileField`/`fileBytes`/`fileName` describe the
  /// single file part. Deliberately a separate method from
  /// postJson/patchJson — those always send `Content-Type: application/json`,
  /// which is wrong for a multipart body (the correct Content-Type,
  /// including the multipart boundary, is set automatically by
  ...
  Future<ApiResult<Map<String, dynamic>>> uploadMultipart(
    ...
    try {
      ...
      final streamedResponse = await _http.send(request).timeout(const Duration(seconds: 30));
      final response = await http.Response.fromStream(streamedResponse);
      if (response.statusCode == 204) return const ApiOk({});
      return _parseEnvelope(response.body);
    } catch (e) {
      _logger.e('api_upload_failed', error: e, stackTrace: StackTrace.current);
      return const ApiErr('NETWORK_ERROR', 'Unable to reach the API service.');
    }
  }
